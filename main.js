let protoWordsArray = [
  { 
    ...
    },
    {
      word: "добрий ранок",
      eng: "Good Morning",
      pro: "./pros/goodmorning.m4a"
    },
    {
      word: "добрий день",
      eng: "Good Afternoon",
      pro: "./pros/goodafternoon.m4a"
    },
    {
      word: "добрий вечір",
      eng: "Good Evening",
      pro: "./pros/goodevening.m4a"
    },
    {
      word: "на добраніч",
      eng: "Good Night",
    ...
    },
    {
      ...
      pro: "./pros/whatisyourname.m4a"
    },
    {
      word: "Мене звуть",
      eng: "My name is",
      pro: "./pros/mynameis.m4a"
    }
    ]
  },
  { 
    ...
    numOfItems: 0,
    items: [{
      word: "Володимир Горовиць",
      eng: "Vladimir Horowitz",
      pro: "./pros/horowitz.m4a"
    },
    {
      word: "Ілля Рєпін",
      eng: "Ilya Repin",
...
//
 ...
 // randomArray = wordsArray[Math.floor(Math.random()*wordsArray.length)];
  ...
  do {
    ...
    console.log ("*length of word is  = " + (wordsArray[randomArray].items[randomWordle].word).length)
    console.log ("*maxchar  = " + maxCharacters)
    count++;
      if ((wordsArray[randomArray].items[randomWordle].word).length < Number(maxCharacters) + 1 ){
      searchingForWord = false;
      console.log("1 set search to false")
    } else {
      if (count > 3500){
        console.log ("count > 3500")
        wordsArray[randomArray].items[randomWordle].word = wordsArray[randomArray].items[randomWordle].word.slice(0, maxCharacters);
 ...
 // console.log(wordsArray[randomArray] + "length of array = " + wordsArray[randomArray].length)
  numofLetters = (wordsArray[randomArray].items[randomWordle].word).length;
//  console.log(wordsArray[randomArray][randomWordle] + " has " + numofLetters + " letters in it")
  wordle = wordsArray[randomArray].items[randomWordle].word;
  wordleEng = wordsArray[randomArray].items[randomWordle].eng;
  audioPro = wordsArray[randomArray].items[randomWordle].pro;
...
allElements.forEach((element) => {
  ...
  function handleSubmitWord(){
    ...
    let checkWordleUpper = checkWordle.toUpperCase();
    const guess = [];
    ...
    guess.forEach(guess => {
      if (!checkWordleUpper.includes(guess.letter.toUpperCase())){
...
//    resultObj.date = date.toString()
    ...
    console.log("guessed word = " + guessedWordUpper + "  wordle = " + wordle)
    let wordleUpper = wordle.toLocaleUpperCase();
    
    if (guessedWordUpper === wordleUpper){
      messageContainerEl.innerText = "Congratulations! The word " + wordleUpper + " means " + wordleEng;
...
function initCategories(){
  ...
  for(i=0; i<protoWordsArray.length; i++){
    ...
    for(j=0; j<protoWordsArray[i].items.length; j++){
      if ((protoWordsArray[i].items[j].word.length) > (maxCharacters)){
